...
/**
 * Created by RmznShn on 11.6.2016.
 */
public class Veritabani extends SQLiteOpenHelper {
    private static String DB_PATH= "data/data/com.example.rmznshn.ceviri_ispanyolca/databases/";
    private static String DB_NAME = "veritabanim";
    private static int DB_SURUM=1;
    private SQLiteDatabase dbObj;
    private final Context context;

    public Veritabani(Context context) {
        super(context,  DB_NAME , null, DB_SURUM);
    ...
    private boolean checkDB(){
        ...
        try{
            ...
            if (checkDB!=null)
            {
                Cursor c= checkDB.rawQuery("SELECT * FROM veritabanim", null);
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE veritabanim (kelimeId INTEGER PRIMARY KEY AUTOINCREMENT,kelimeTr TEXT,kelimeIs TEXT);");
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL("DROP TABLE If EXIST veritabanim");
        onCreate(db);
    }
}